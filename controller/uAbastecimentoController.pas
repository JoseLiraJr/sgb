...
type
  ...
  private
    ...
    procedure SetVlrsObjeto(ObjAbastecimento : TAbastecimento; dtSource : TDataSource);
  public
...
begin
  ...
  SetVlrsObjeto(Abastecimento, dtSource);
end;
...
procedure TAbastecimentoController.SetVlrsObjeto(
  ObjAbastecimento: TAbastecimento; dtSource: TDataSource);
begin
  with dtSource.DataSet, ObjAbastecimento do
   begin
     IdBomba := FieldByName('ID_BOMBA').AsInteger ;
     PercImposto := FieldByName('PERC_IMPOSTO').AsFloat;
     VlrLiquido := FieldByName('VLR_LIQUIDO').AsFloat;
     VlrImposto := FieldByName('VLR_IMPOSTO').AsFloat;
     VlrTotal := FieldByName('VLR_TOTAL').AsFloat;
     QtdCombustivel := FieldByName('QTD_COMBUSTIVEL').AsFloat;
     DtAbastecimento := FieldByName('DT_ABASTECIMENTO').AsDateTime;
   end;
end;
